                    ...
                    with st.spinner("🔮 Analyzing image and generating content..."):
                        try:
                            ...
                            if caption and sentiment_result.get('success'):
                                # Generate hashtags
                                # Engine hashtags (trending database)
                                engine_tags = hashtag_engine.get_hashtags_by_priority(
                                    category=sentiment_result['category'],
                                    sentiment=sentiment_result['sentiment'],
                                    all_sentiments=sentiment_result.get('all_sentiments', {})
                                )

                                # Gemini AI hashtags (image aware)
                                ai_tags = []
                                if st.session_state.use_gemini and st.session_state.gemini_analyzer:
                                    ai_tags = st.session_state.gemini_analyzer.generate_hashtags(
                                        image,
                                ...
                                # Combine both
                                hashtags = list(dict.fromkeys(ai_tags + engine_tags))[:num_hashtags]
